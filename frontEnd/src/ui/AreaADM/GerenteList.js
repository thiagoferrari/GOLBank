...
export default function () {
	...
	async function deleteItem() {
		try {
			await axios.delete(`http://localhost:3001/gerentes/${deletable}`)
	...
	const columns = [
		...
		},
		{
			...
			flex: true,
			renderCell: params => (
				<IconButton onMouseDown={() => history.push(`/gerentes/edit/${params.id}`)}>
					<EditIcon />
				</IconButton>
			)
		},
	...
	return (
		<>
			<ConfirmDialog isOpen={dialogOpen} onClose={handleDialogClose}>
				Deseja realmente excluir esta Instrução?
			</ConfirmDialog>
			...
			<h1 onClick={() => getData()} style={{ textAlign: 'center' }}>Listagem de Gerentes</h1>
			<Toolbar className={classes.toolbar}>
				<Button color="secondary" variant="contained"
					size="large" startIcon={<AddBoxIcon />}
					onMouseDown={() => history.push(`/gerentes/novo`)}>
					add Novo Gerente
				</Button>
			</Toolbar>
			<Paper elevation={4}>
